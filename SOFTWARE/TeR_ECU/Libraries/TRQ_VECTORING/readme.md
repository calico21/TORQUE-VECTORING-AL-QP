...
Para actuar, medimos el slip real filtrado y, si nos pasamos del target, entra un controlador PI. Detalle clave: La salida de este PI pasa por una función matemática softplus que garantiza por diseño que el TC solo puede restar par, jamás añadirlo. Como red de seguridad final contra vibraciones, un filtro inercial vigila la derivada de la velocidad de la rueda para cortar el accel en milisegundos si saltamos por un piano.

Requisitos de Sensores: Para que esto no explote, el archivo de interfaz (gp_interface.c) necesita datos limpios del bus a 200Hz. Es  obligatorio mapear: Aceleración longitudinal (a_x) y lateral (a_y).Yaw rate de la IMU (w_z).Posición de pedales (acelerador y presión de freno).Ángulo de la dirección (steering).Velocidad angular en bruto de CADA inversor trasero (sin las RPM individuales, el RLS es ciego y el TC no hace nada).Telemetría Avanzada (El Muro de Boxes)El código empaqueta la física interna en 3 tramas CAN dedicadas para que no vayamos a ciegas cuando lo subamos a los caballetes o lo metamos en pista. 
